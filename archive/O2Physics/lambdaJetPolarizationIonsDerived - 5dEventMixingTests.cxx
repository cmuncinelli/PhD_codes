...
struct lambdajetpolarizationionsderived {
  ...
  struct : ConfigurableGroup {
    ...
    Configurable<bool> doMixedEventJet{"doMixedEventJet", false, "Sample a jet-proxy direction from a real, similar OTHER collision (O2 event mixing), instead of a fitted distribution. Currently covers the leading-particle proxy only -- see the comment above the SameKindPair construction in processPolarizationData()."};
    Configurable<int> mixedEventWindowSize{"mixedEventWindowSize", 5, "categoryNeighbours for the event-mixing pool: how many same-bin collisions stay eligible as mixing partners at once. Larger = better preserves the a posteriori correlations between the 5 mixing variables, at the cost of a larger/slower pool."};
  ...
  void init(InitContext const&)
  {
    ...
    if (calculateEventMixingHists) {
      histos.add("JetKinematicsQA/h5dLeadJetEtaPhiPtPVzCent", "h5dLeadJetEtaPhiPtPVzCent;#eta;#phi;p_{t};Primary Vertex Z [cm];Centrality (%);Counts", THnSparseF,
      ...
      histos.add("JetKinematicsQA/h5dSubLeadJetEtaPhiPtPVzCent", "h5dSubLeadJetEtaPhiPtPVzCent;#eta;#phi;p_{t};Primary Vertex Z [cm];Centrality (%);Counts", THnSparseF,
        {axisConfigurations.axisEtaCoarse, axisConfigurations.axisPhi, axisConfigurations.axisJetPt, axisConfigurations.axisPVz, axisConfigurations.axisCentrality});
      histos.add("JetKinematicsQA/h5dLeadPEtaPhiPtPVzCent", "h5dLeadPEtaPhiPtPVzCent;#eta;#phi;p_{t};Primary Vertex Z [cm];Centrality (%);Counts", THnSparseF,
        {axisConfigurations.axisEtaCoarse, axisConfigurations.axisPhi, axisConfigurations.axisJetPt, axisConfigurations.axisPVz, axisConfigurations.axisCentrality});
    }

    // doMixedEventJet QA: gauge the size of the "too few collisions per bin" problem (see resonanceMergeDF.cxx)
    // before reaching for anything more expensive than the plain LUT/SameKindPair approach below.
    if (fakePolSwitches.doMixedEventJet) {
      histos.add("JetKinematicsQA/hMixedEventJetOutcome", "hMixedEventJetOutcome;Outcome (0=skipped, 1=found);Counts", kTH1D, {{2, -0.5f, 1.5f}});
      // Bin range assumes mixedEventWindowSize is O(20) or less; widen if you configure a much larger window.
      histos.add("JetKinematicsQA/hMixedEventJetWindowNeighbours", "hMixedEventJetWindowNeighbours;Neighbours found in bin window;Counts", kTH1D, {{22, -0.5f, 21.5f}});
    }
  ...
  /// \param hasMixedPartner,mixedEta,mixedPhi for doMixedEventJet: a real mixing-partner direction, pre-resolved by the
  ///        caller from the SameKindPair-built lookup table (see processPolarizationData()). If hasMixedPartner is false
  ///        (no partner found for this collision, or mixing isn't implemented yet for this proxy type), this is a no-op --
  ///        the fallback is to skip, i.e. leave the real, undistorted proxy untouched (unlike forcePreviousJet's cache-invalidation fallback).
  ...
  inline void applyProxyDistortion(bool& hasValidProxy, float& pt, float& eta, float& phi, XYZVector& unitVec, 
                            ...
                            bool hasMixedPartner, float mixedEta, float mixedPhi){
    if (!fakePolSwitches.forcePerpToJet && !fakePolSwitches.forceJetDirectionSmudge && !fakePolSwitches.forceRandJet && !fakePolSwitches.forcePreviousJet && !fakePolSwitches.forceDatalikeJet && !fakePolSwitches.doMixedEventJet)[[likely]] {
    ...
    } else if (fakePolSwitches.doMixedEventJet) {
      // Real event mixing (see the SameKindPair construction in processPolarizationData()):
      // borrow the direction of a real, similar OTHER collision's proxy, rather than sampling a fitted distribution.
      // Mirrors forceDatalikeJet/forcePreviousJet above: only unitVec is rebuilt, eta/phi are intentionally left as
      // this collision's own real values (matching forcePreviousJet's existing convention just above).
      if (hasMixedPartner) {
        const double inverseCoshEta = 1.0 / std::cosh(mixedEta);
        const double sinPhi = std::sin(mixedPhi);
        const double cosPhi = std::cos(mixedPhi);
        unitVec = XYZVector(cosPhi * inverseCoshEta, sinPhi * inverseCoshEta, std::tanh(mixedEta));
      }
      // else: no mixing partner (sparse bin, or mixing not yet implemented for this proxy type) -- fallback is to skip.
    }
                               ...
                               o2::aod::RingLeadPs const& leadPs)
  {
    ...
    // ------------------------------------------------------------------
    // doMixedEventJet: real event mixing via O2's SameKindPair machinery.
    // Built once per dataframe (like prevJetCache above -- not meant to
    // interact with nProxyResamples, same restriction as that cache),
    // consumed by applyProxyDistortion() through the LUT below.
    //
    // Currently covers the leading-particle proxy only. RingLeadPs has at
    // most one row per collision, so it slots directly into the standard
    // SameKindPair<Collisions, Associated, BinningPolicy> pattern (the same
    // role Tracks plays in the O2 tutorial's event-mixing examples). Leading
    // jets are deferred: RingJets can have several rows per collision, so
    // mixing on it needs its own "reduced to one leading jet per collision"
    // local table first -- not built yet, hence the (false, 0.f, 0.f)
    // passed at the leadJet/subJet applyProxyDistortion() call sites below.
    //
    // Binning: 1 plain RingCollisions column (PVz) + 4 lambda-derived
    // dimensions. Centrality has to be a lambda rather than a plain column
    // because a plain column can't respect the runtime centralityEstimator
    // choice; the 3 leadP dimensions have to be lambdas because they live
    // on the sparse associated table, not on RingCollisions itself. Each
    // leadP lambda returns -999 (well outside any real axis range) for
    // collisions with no leading particle, so ignoreOverflows=true excludes
    // them from binning/pairing entirely -- no manual "does this collision
    // have a leadP" branching needed anywhere here.
    //
    // To add Interaction Rate as a 6th dimension later: one more axis (e.g.
    // a new axisConfigurations.axisIRBinning), one more plain-column entry
    // (RingCollisions::InteractionRate is already a column, so it can join
    // Zvtx as a plain column -- no new lambda needed), same pattern as PVz.
    // ------------------------------------------------------------------
    ...
    if (fakePolSwitches.doMixedEventJet) {
      ...
      auto getMixLeadPPt = [&leadPs, &mixCache](o2::aod::RingCollision const& col) -> float {
        auto rows = leadPs.sliceByCached(o2::aod::lambdajetpol::ringCollisionId, col.globalIndex(), mixCache);
        return rows.size() > 0 ? rows.begin().leadParticlePt() : -999.f; // sentinel -> overflow -> never paired
      };
      auto getMixLeadPEta = [&leadPs, &mixCache](o2::aod::RingCollision const& col) -> float {
        auto rows = leadPs.sliceByCached(o2::aod::lambdajetpol::ringCollisionId, col.globalIndex(), mixCache);
        return rows.size() > 0 ? rows.begin().leadParticleEta() : -999.f;
      };
      auto getMixLeadPPhi = [&leadPs, &mixCache](o2::aod::RingCollision const& col) -> float {
        auto rows = leadPs.sliceByCached(o2::aod::lambdajetpol::ringCollisionId, col.globalIndex(), mixCache);
        return rows.size() > 0 ? rows.begin().leadParticlePhi() : -999.f;
      };
      auto getMixCentrality = [this](o2::aod::RingCollision const& col) -> float {
        return this->getCentrality(col);
      };

      using MixBinningType = FlexibleBinningPolicy<std::tuple<decltype(getMixLeadPPt), decltype(getMixLeadPEta), decltype(getMixLeadPPhi), decltype(getMixCentrality)>,
                                                    o2::aod::lambdajetpol::Zvtx, decltype(getMixCentrality),
                                                    decltype(getMixLeadPPt), decltype(getMixLeadPEta), decltype(getMixLeadPPhi)>;
      MixBinningType mixBinning{{getMixLeadPPt, getMixLeadPEta, getMixLeadPPhi, getMixCentrality},
                                 {axisConfigurations.axisPVz, axisConfigurations.axisCentrality,
                                  axisConfigurations.axisJetPt, axisConfigurations.axisEtaCoarse, axisConfigurations.axisPhi},
                                 true};

      SameKindPair<o2::aod::RingCollisions, o2::aod::RingLeadPs, MixBinningType> mixPair{
          mixBinning, fakePolSwitches.mixedEventWindowSize, -1, collisions, std::make_tuple(leadPs), &mixCache};

      // "Last neighbour wins" if a collision appears in more than one pair within its
      // window -- a simple, deterministic choice, not a uniform-random pick among the
      // window's members. Revisit if that matters for your systematics.
      for (auto it = mixPair.begin(); it != mixPair.end(); ++it) {
        auto& [c1, leadP1, c2, leadP2] = *it;
        if (leadP1.size() > 0 && leadP2.size() > 0) { // should always hold, given the sentinel-driven overflow exclusion above
        ...
        if (it.isNewWindow()) { // count each bin-window once, not once per pair inside it
          histos.fill(HIST("JetKinematicsQA/hMixedEventJetWindowNeighbours"), it.currentWindowNeighbours());
        }
      }
    }
    ...
    for (auto const& collision : collisions) {
      ...
      if (hasValidLeadingP) {
        ...
        // doMixedEventJet: resolve this collision's mixing partner (if any) from the LUT built above.
        bool hasMixedLeadP = false;
        float mixedLeadPEta = 0.f, mixedLeadPPhi = 0.f;
        if (fakePolSwitches.doMixedEventJet) {
          auto itMix = mixedLeadPByCollision.find(collId);
          hasMixedLeadP = (itMix != mixedLeadPByCollision.end());
          if (hasMixedLeadP) {
            mixedLeadPEta = itMix->second.eta;
            mixedLeadPPhi = itMix->second.phi;
          }
          histos.fill(HIST("JetKinematicsQA/hMixedEventJetOutcome"), hasMixedLeadP ? 1 : 0);
        }
        ...
        applyProxyDistortion(hasValidLeadingP, leadPPt, leadPEta, leadPPhi, leadPUnitVec, 
                             ...
                             etaLeadPDist, phiLeadPDist, rng, hasMixedLeadP, mixedLeadPEta, mixedLeadPPhi);
      ...
      if (hasValidLeadingJet) {
        ...
        applyProxyDistortion(hasValidLeadingJet, leadingJetPt, leadingJetEta, leadingJetPhi, leadingJetUnitVec, 
                             ...
                             false, 0.f, 0.f); // doMixedEventJet: leading-jet mixing not implemented yet (see LUT comment above) -- always "no partner"
      ...
      if (hasValidSubJet) {
        ...
        applyProxyDistortion(hasValidSubJet, subleadingJetPt, subleadingJetEta, subleadingJetPhi, subJetUnitVec, 
                             ...
                             etaLeadPDist, phiLeadPDist, rng,
                             false, 0.f, 0.f); // doMixedEventJet: subleading-jet mixing not implemented yet (see LUT comment above) -- always "no partner"
