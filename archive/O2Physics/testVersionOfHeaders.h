...
DECLARE_SOA_INDEX_COLUMN(RingCollisions, ringCollision); // Now using index column because collisions are cheap to store.
// DECLARE_SOA_COLUMN(CollIdx, collIdx, uint64_t); // Did not use an SOA_INDEX_COLUMN because the collisions table is only filled when there is a valid V0 in the collision.
//                                                 // In other words, there might be a jet saved in a collision that has no V0s (and thus was not saved in RINGCOLLISIONS),
//                                                 // and then that jets column would try to link to an inexistent collision index_column and fail.
