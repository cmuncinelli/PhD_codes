...
void RunWorker(Pythia* pythiaPtr, int WorkerId, const std::string output_folder, const std::string input_card_name){
  	// Pythia pythia;
	...
	// Access the event directly -- No more event loops!
    Event& event = pythiaPtr->event;
	
	ntrack = event.size(); // This could be a bit problematic to estimate multiplicity: this includes non-final particles!
	...
	for (int i = 0; i < event.size(); ++i){
		...
		Int_t daughter1 = event[i].daughter1();
		Int_t daughter2 = event[i].daughter2();
		...
		Float_t rapidity = event[i].y();
		Float_t eta_rap = event[i].eta();
		Bool_t isCharged = event[i].isCharged();
		...
		int particle_PID = event[i].id();
		ID[i] = particle_PID;
		int motherIdx1 = event[i].mother1(); // Doing this, I can access it only once and don't need to re-read this information for pions!
		int motherIdx2 = event[i].mother2();
		...
		if (motherIdx1 > 0 && motherIdx2 == 0){
			if (event[motherIdx1].tau0() >= 10.){ // Tau0 in mm/c. Reference is slide 112 of https://indico.cern.ch/event/666222/contributions/2768780/attachments/1551303/2437229/DPG_AnalysisTutorial_20171102.pdf
				IsPrimary[i] = false;
			}
			else{
				Ntracks_primary += 1;
...
int main(int argc, char *argv[]){
	...
	// Create and configure PythiaParallel
    PythiaParallel pythia;
	pythia.readString("Parallelism:numThreads = " + std::to_string(N_cores));
	pythia.readString("Parallelism:processAsync = on"); // I manually prevent race conditions in the code by using an individual file/histogram for each core!
    pythia.readString("Main:numberOfEvents = " + std::to_string(N_ev));
	...
	// Run once - the callback signature must be void(Pythia*):
    pythia.run([&](Pythia* pythiaPtr) {
        // find which underlying instance this is (0..numThreads-1)
        int WorkerId = pythiaPtr->settings.mode("Parallelism:index");
        // process the already-generated event
        RunWorker(pythiaPtr, WorkerId, output_folder, input_card_name);
    });
	// RunWorker(WorkerId, N_ev_current_worker, output_folder_for_current_card, input_card_path, input_card_name); // Older structure!
