...
void RunWorker(const int WorkerId, const int N_ev, const std::string output_folder, const std::string input_card_path, const std::string input_card_name){
	...
	for (int iEvent=0; iEvent<N_ev; ++iEvent){ // Could have done this with a "while", but do notice that this loop runs until the target N_ev is reached in *useful* events.
		...
		const auto &pythia_event = pythia.event; // A shorthand to avoid calling repeated lookups this a bunch of times
		...
		for (int i = 0; i < pythia_event.size(); ++i){
			if (pythia_event[i].idAbs() == 3122 && std::fabs(pythia_event[i].eta()) < ALICE_charged_particle_acceptance){
		...
		if (!contains_lambda_or_lambdabar_in_measurable_eta){
			...
			// event_total_particles_processed_useless_ev += pythia_event.size();
			// N_useless_ev += 1;
			iEvent--;
			continue;
		}
		...
		ntrack = pythia_event.size(); // This could be a bit problematic to estimate multiplicity: this includes non-final particles!
		...
		for (int i = 0; i < ntrack; ++i){
			const auto &particle = pythia_event[i]; // Just a fancy alias, because it is a reference (&particle), so there is no copy actually being done here.
			...
			// 	if (pythia_event[motherIdx1].tau0() >= 10.){ // Tau0 in mm/c. Reference is slide 112 of https://indico.cern.ch/event/666222/contributions/2768780/attachments/1551303/2437229/DPG_AnalysisTutorial_20171102.pdf
			...
			if (isfinal){
				...
				if (isCharged){
					...
					if (std::abs(particle_PID) == 2212){
						...
						// Particle mother_particle_object = pythia_event[motherIdx1]; // Already using the Pythia8 namespace
						const int mother_PID = pythia_event[motherIdx1].id(); // Removed the mother_particle_object as it was no longer being used
						if (std::abs(mother_PID) == 3122){
							...
							while ((pythia_event[temp_mother_idx].mother1() == pythia_event[temp_mother_idx].mother2()) && (pythia_event[temp_mother_idx].mother1() > 0)){
								...
								// mother_particle_object = pythia_event[mother_particle_object.mother1()];
								temp_mother_idx = pythia_event[temp_mother_idx].mother1(); // Quicker than updating the whole object: just updates the index!
							}

								// Getting the mother of the Lambda:
							Particle lambda_mother_object = pythia_event[pythia_event[temp_mother_idx].mother1()];
							...
							if ((std::abs(lambda_mother_object.status()) >= 81 && std::abs(lambda_mother_object.status()) <= 86) || (std::abs(lambda_mother_object.status()) >= 101 && std::abs(lambda_mother_object.status()) <= 106)){
							...
							const auto &mother = pythia_event[motherIdx1]; // Just a shorthand to avoid lookup
							LambdaOrLBar_pt.push_back(mother.pT());
							LambdaOrLBar_y.push_back(mother.y());
							LambdaOrLBar_Phi.push_back(mother.phi());
							// LambdaOrLBar_m.push_back(pythia_event[motherIdx1].m());
		...
		for (const auto& jet : sortedJets){
			...
			// 	const int particle_PID = pythia_event[particle_PythiaIdx].id();
		...
		// 	int particle_PID = pythia_event[particle_PythiaIdx].id();
