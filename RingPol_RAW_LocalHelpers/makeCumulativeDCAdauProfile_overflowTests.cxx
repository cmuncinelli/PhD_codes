...
TProfile2D* MakeCumulativeProfile2D(TProfile2D* pDiff, const char* outName, const char* outTitle) {  
    ...
    for (int i = 1; i <= nPhi; i++) {
        ...
        // Seed with the DCA-axis OVERFLOW bin: candidates with DCA above the
        // histogram's upper edge satisfy "DCA > threshold" for EVERY visible
        // threshold, so they belong in every cumulative bin, starting from
        // the very first (loosest) one - otherwise bin 1 silently undercounts
        // and won't match the total entries seen by a differently-binned
        // DCA variable.
        int overflow_bin = pDiff->GetBin(i, nDCA + 1);
        double sum_N  = pDiff->GetBinEntries(overflow_bin);         // -> bin entries
        double sum_Y  = sum_N * pDiff->GetBinContent(overflow_bin); // -> bin content (raw sum of Y)
        double sum_Y2 = pDiff->GetSumw2()->At(overflow_bin);        // -> bin sum-of-squares
...
TProfile* MakeCumulativeProfile1D(TProfile2D* pDiff, const char* outName, const char* outTitle) {
    ...
    // Seed with the DCA-axis OVERFLOW row: candidates with DCA above the
    // histogram's upper edge satisfy "DCA > threshold" for EVERY visible
    // threshold, so they belong in every cumulative bin, starting from the
    // very first (loosest) one - otherwise bin 1 silently undercounts and
    // won't match the total entries seen by a differently-binned DCA variable.
    ...
    for (int i = 1; i <= nPhi; i++) {
        int overflow_bin = pDiff->GetBin(i, nDCA + 1);
        double n_of = pDiff->GetBinEntries(overflow_bin);
        total_sum_N  += n_of;
        total_sum_Y  += n_of * pDiff->GetBinContent(overflow_bin);
        total_sum_Y2 += pDiff->GetSumw2()->At(overflow_bin);
    }
...
TH2D* MakeCumulativeTH2D(TH2D* hDiff, const char* outName, const char* outTitle) {
    ...
    for (int i = 1; i <= nPhi; i++) {
        // Seed with the DCA-axis OVERFLOW bin, for the same reason as the
        // profile helpers above: those counts belong in every cumulative
        // bin, since DCA above the axis max exceeds every visible threshold.
        int overflow_bin = hDiff->GetBin(i, nDCA + 1);
        double sum_Counts = hDiff->GetBinContent(overflow_bin);
        double overflow_err = hDiff->GetBinError(overflow_bin);
        double sum_Err2   = overflow_err * overflow_err;
        for (int j = nDCA; j >= 1; j--) {
