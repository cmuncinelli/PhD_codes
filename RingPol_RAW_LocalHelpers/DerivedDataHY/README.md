...
have already been downloaded -- so it is worth checking upfront. `hadd` is also
required by Step 7 of `run_all_wagons.sh`, which merges its parallel batch
outputs; that step is the only part of the chain outside the download workflow
that needs the alienv.
...
|       |-- results_CumulativePlots/        # makeCumulativeDCAdauProfile.cxx output + logs/
|       +-- results_AuxPerConfig/           # auxiliaryPerConfigPlots.cxx output + logs/
+-- LHC25ae_pass2_small/
...
- `--skip-forensics` (`-f`) skips Step 7. Useful when the AO2Ds have already
  been checked, or when they are no longer on disk.
...
- Run `zvtxBitForensics.cxx` (ROOT, parallel) over the wagon's `AO2Ds/`

Steps 2, 3, 4, and 5 are skipped automatically if step 1 fails for a given pair.
Step 7 reads the raw AO2Ds rather than the consumer output, so it runs
independently of that outcome. It is skipped -- not failed -- when `AO2Ds/` is
absent or empty, which is the normal situation once the AO2Ds have been deleted
...
> **Note:** `results_SigExtract/`, `results_DeltaErr/`, `results_CumulativePlots/`,
> and `results_AuxPerConfig/` are *not* scaffolded by this script. Each is created
> automatically (recursively, including its own `logs/` subfolder) the first time
> the corresponding ROOT macro runs, so there is nothing to add here manually.
...
1. `runDerivedDataConsumer_HY.sh` -- produces `results_consumer/ConsumerResults_<SUFFIX>.root`
2. `extractDeltaErrors.cxx` (ROOT) -- delta/error plots from the consumer output, written to `results_DeltaErr/`
3. `signalExtractionRing.cxx` (ROOT) -- signal extraction, output in `results_SigExtract/`
4. `makeCumulativeDCAdauProfile.cxx` (ROOT) -- cumulative DCA profiles QA, written to `results_CumulativePlots/`
...
> **Note on Step 7 (AO2D forensics):**
> Unlike every other step, this one reads the raw AO2Ds in the wagon's `AO2Ds/`
> folder rather than any consumer output, so it also runs under
...
Steps 2, 4, and 5 each write into their own dedicated sibling folder of `results_consumer/`
instead of writing back into it, so the consumer's own output folder stays limited to
`ConsumerResults_<SUFFIX>.root` and its own logs/config snapshots. This mirrors the
`results_SigExtract/` convention step 3 already used before this change:

| Step | Macro | Output folder |
| --- | --- | --- |
| 2 | `extractDeltaErrors.cxx` | `results_DeltaErr/` |
| 3 | `signalExtractionRing.cxx` | `results_SigExtract/` |
| 4 | `makeCumulativeDCAdauProfile.cxx` | `results_CumulativePlots/` |
| 5 | `auxiliaryPerConfigPlots.cxx` | `results_AuxPerConfig/` |

Each of these four macros now takes the output directory as an explicit second
command-line argument (`<exe> <inputFilePath> <outputDirectory>`) and creates it
recursively if it does not already exist -- so nothing needs to be scaffolded
by hand beforehand.
...
All stdout from child processes is suppressed to keep the terminal readable. Each step's
log now lives under its own `results_*/logs/` folder, alongside that step's own output
