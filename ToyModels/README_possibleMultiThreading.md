...
| `--jobs N` | `-j` | Override the maximum number of concurrent generator jobs (one per registered run). |
| `--multiply M` | `-m` | Split each job into M NUMA-pinned shards and merge with `hadd`. Default: 1 (no sharding). See NUMA section below. |
...
# Run all families (default: no sharding):
./runHelicityToyModel.sh

# Use both NUMA nodes for ~2x throughput (recommended on jarvis15):
./runHelicityToyModel.sh --multiply 2

# Run only one specific family with NUMA sharding:
./runHelicityToyModel.sh --family 1 --multiply 2
...
The `--multiply M` flag splits each registered job into M independent generator
sub-processes (shards).  Each shard processes N/M events with a unique random
seed and is pinned to a dedicated NUMA node using `numactl --cpunodebind=K --membind=K`.
After all M shards finish, `hadd` merges their ROOT files into the final output
file and the per-shard files are deleted.  The plotter then runs once on the merged result.

**On jarvis15 (AMD EPYC 9634, dual-socket, 84 physical cores per socket):**

```
NUMA node 0: CPUs 0-83,   168-251  (84 physical cores, 168 logical with HT)
NUMA node 1: CPUs 84-167, 252-335  (84 physical cores, 168 logical with HT)
```

With `--multiply 2` and ~79 registered jobs, the script launches 158 generator
processes simultaneously.  Each NUMA node runs ~79 processes across its 84 physical
cores, saturating available compute without cross-socket memory traffic.

**Statistical correctness:** all histogram types in the generator output are
correctly merged by `hadd`:

- `TH1D` / `TH2D`: `hadd` sums bin contents and `Sumw2` arrays.  **Exact.**
- `TProfile` (written via flat accumulators + `SetBinContent` / `SetBinEntries`
  + `ResetStats()`): `hadd` sums `fArray` (sum\_wy) and `fBinEntries` (sum\_w)
  per bin, giving `GetBinContent = total_sum_wy / total_sum_w`, the correct
  weighted mean.  ROOT's `Add()` (called internally by `hadd`) also rebuilds
  the global statistics cache (`fTsumwy`, `fTsumw`, ...), so the statsbox
  remains correct after the merge -- no additional `ResetStats()` needed.
  **Exact.**

**Seed policy:** to guarantee independent random streams between shards,
each shard receives a unique seed:
- If `SEED != 0`: shard i gets `SEED * 1000 + i`.  Neighboring job base seeds
  differ by 1000, ensuring no seed collisions between jobs as long as MULTIPLY < 1000.
- If `SEED == 0` (time-based): M seeds are drawn from `/dev/urandom` so that
  near-simultaneous processes do not risk identical `TRandom3(0)` auto-seeds.

**Recommended value:** `--multiply 2` for this dual-socket machine.
`--multiply 3` would over-subscribe the physical cores via hyperthreading; since the generator is compute-bound (not memory-bound), this may slow things down.
