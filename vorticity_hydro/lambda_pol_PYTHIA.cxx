...
int main(int argc, char *argv[]){
    ...
    double N_ev_receiver = atof(argv[2]); // This receives input like 1e9 and converts it into a proper double.
	uint N_events = static_cast<int>(N_ev_receiver);

    std::cout << "\nReading results for input card " << input_card_name << " and running for " << N_events << " events";
    ...
    /////////////////////////////////////////////////////////////////////
    std::cout << "Initializing variables and declaring histograms" << std::endl;
    ...
    auto hLambdaCounter_minus1_pT_y_DeltaPhiJ = new TH3D("hLambdaCounter_pT_y_DeltaPhiJ", "hLambdaCounter_pT_y_DeltaPhiJ", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    ...
    auto hHighestMomentum = new TH1D("hHighestMomentum", "hHighestMomentum", 100, 0, 10); // To know the distribution of the jet proxies' momenta
    auto hHighestMomentumPzOverNorm = new TH1D("hHighestMomentumPzOverNorm", "hHighestMomentumPzOverNorm", 100, -1, 1); // Tells me how much of the momentum of the highest momentum particles is in Z (in normalized vector units)
    ...
    std::cout << "The Ntracks_final_charged multiplicity cuts for centrality definition in each class are:" << std::endl;
    for (int i = 0; i < 9; i++){
    ...
    Int_t Ntracks_final_charged;
        // PID selection:
    Int_t PID[kMaxTrack];
    Int_t mother1_Idx[kMaxTrack]; // Not quite the PID of the mother. It is the Idx of the mother in Pythia's list of particles
    Int_t mother2_Idx[kMaxTrack];
    Bool_t IsFinal[kMaxTrack];
    Bool_t IsCarbonCopy[kMaxTrack];
        // 4-moment information:
    Float_t m[kMaxTrack]; // The actual simulated mass of the particle in the dynamic, so may be somewhat different from the PDG value.
    ...
    chain.SetBranchAddress("ID", PID); // Do notice that you don't want to pass the pointer address here, but just the variable itself (which is a pointer), so don't use "&"!
    chain.SetBranchAddress("mother1_ID", mother1_Idx);
    chain.SetBranchAddress("mother2_ID", mother2_Idx);
    chain.SetBranchAddress("IsFinal", IsFinal);
    chain.SetBranchAddress("IsCarbonCopy", IsCarbonCopy);
    ...
    for (Long64_t iEvent = 0; iEvent < nEntries; iEvent++){
        ...
        Bool_t contains_particle = false;
        for (int particle_idx = 0; particle_idx < ntrack; particle_idx++){
            ...
            if (IsCarbonCopy[particle_idx]){continue;}
            ...
            if (highest_particle_momentum < particle_momentum){
            ...
            TLorentzVector lambda_4_momentum(lambda_px, lambda_py, lambda_pz, lambda_E);
            TLorentzVector proton_4_momentum(particle_px, particle_py, particle_pz, particle_E);
            ...
            lambda_4momenta.push_back(lambda_4_momentum);
            proton_4momenta.push_back(proton_4_momentum);
        ...
        // N_useful_events += 1;
        hNUsefulEv->Fill(0);
            // Getting the jet variables to calculate the ring observable!
        ...
        hHighestMomentum->Fill(highest_particle_momentum);
        hHighestMomentumPzOverNorm->Fill((jet_trimomentum.Unit()).Z()); // How much of the unit vector is in z. Only for |y| < 0.5 jets
        ...
        for (int i = 0; i < proton_4momenta.size(); i++){ // Goes over all protons and their trimomenta
            ...
            double rotated_proton_phi = wrapToInterval(proton_4momentum.Phi() - jet_phi, 0, 2*PI);
            ...
            hLambdaCounter_DeltaPhiJRingAngles->Fill(rotated_lambda_phi);
            ...
            if (lambda_pT > 0.5 && lambda_pT < 1.5){
                ...
                hProtonStarCounter_DeltaPhiJRing_PtCuts->Fill(rotated_lambda_phi);
            }
        }
    }
    ...
    ////////////////////////////////////////////////////////////
    std::cout << "Now averaging histograms" << std::endl;
    ...
    output_file_path += "_40_50_only_" + std::to_string(N_events) + "evInAllClasses.root"; // Split in two to make the "string + const char*" operation work
    TFile f(output_file_path.c_str(), "RECREATE");

    hHighestMomentum->Write();
    hHighestMomentumPzOverNorm->Write();
    hNUsefulEv->Write();
...
int FindClosestBin(TH1D* hist, double targetValue){
    ...
    double previousDiff = std::abs(hist->GetBinContent(closestBin_idx) - targetValue);

    // std::cout << "Starting difference: " << previousDiff << std::endl;

    for (int i = nBins - 1; i >= 1; i--){
        double currentContent = hist->GetBinContent(i);
        double currentDiff = std::abs(currentContent - targetValue);
