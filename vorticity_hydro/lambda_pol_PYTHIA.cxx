...
int main(int argc, char *argv[]){
    if (argc != 6){
		std::cerr << "Usage:" << argv[0] << " simulations_folder output_folder_char input_card_name N_events target_centrality_bin" << std::endl;
        return 1; // Indicate an error.
    }

	const char *simulations_folder = (const char*) argv[1];
    const char *output_folder_char = (const char*) argv[2];
    const char *input_card_name = (const char*) argv[3];
    double N_ev_receiver = std::atof(argv[4]); // This receives input like 1e9 and converts it into a proper double.
    ...
    int target_centrality_bin = std::atoi(argv[5]);
    ...
    std::string output_folder = output_folder_char;
