...
inline int mapGlobalToCompact(const TH1& h, int globalBin); // Receives the object itself, not the pointer!
...
int main(int argc, char *argv[]){ // Changed the code into a compiler-friendly way, that searches for a "main()" function instead of a main with the same name as the .cxx file that ROOT's .x compiler expects
    ...
    // Declaring buffer vectors:
    std::vector<double> hLambdaCounter_pT_y_phi_Weighted_bufferGlobal(N_bins_total, 0.0);
    std::vector<double> hLambdaAvgDotX_pT_y_phi_Weighted_bufferGlobal(N_bins_total, 0.0);
    std::vector<double> hLambdaAvgDotY_pT_y_phi_Weighted_bufferGlobal(N_bins_total, 0.0);
    std::vector<double> hLambdaAvgDotZ_pT_y_phi_Weighted_bufferGlobal(N_bins_total, 0.0);
    std::vector<double> hLambdaCounterSquared_pT_y_phi_Weighted_bufferGlobal(N_bins_total, 0.0);
    ...
    std::vector<double> hLambdaPolY_phi_pT_Weighted_bufferGlobal(N_bins_phi*N_bins_pT, 0.0);
    std::vector<double> hLambdaPolZ_phi_pT_Weighted_bufferGlobal(N_bins_phi*N_bins_pT, 0.0);
    std::vector<double> hLambdaCounter_phi_pT_Weighted_bufferGlobal(N_bins_phi*N_bins_pT, 0.0);
    std::vector<double> hLambdaCounter_phi_Weighted_bufferGlobal(N_bins_phi, 0.0);

    std::vector<double> hLambdaCounter_phiRingAngles_Weighted_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hProtonCounter_phiRing_Weighted_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hProtonStarCounter_phiRing_Weighted_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hDebugCounter_phi_star_sampler_bufferGlobal(N_bins_phi, 0.0);

    std::vector<double> hLambdaCounter_phi_Weighted_PtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hLambdaCounter_phiRingAngles_Weighted_PtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hProtonCounter_phiRing_Weighted_PtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hProtonStarCounter_phiRing_Weighted_PtYCuts_bufferGlobal(N_bins_phi, 0.0);

    std::vector<double> hLambdaPolX_phi_Weighted_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hLambdaPolY_phi_Weighted_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hLambdaPolZ_phi_Weighted_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hLambdaPolX_pT_y_phi_Weighted_bufferGlobal(N_bins_total, 0.0);
    std::vector<double> hLambdaPolY_pT_y_phi_Weighted_bufferGlobal(N_bins_total, 0.0);
    std::vector<double> hLambdaPolZ_pT_y_phi_Weighted_bufferGlobal(N_bins_total, 0.0);
    std::vector<double> hRingObservable_proxy_from_daughter_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_proxy_from_daughter_eq_def_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_proxy_from_daughter_star_eq_def_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_TrueValue_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_TrueValuePtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_NormPolTrueValuePtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_TrueValuePtYCuts_integrated_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_NormPolTrueValuePtYCuts_integrated_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_proxy_from_daughter_eq_defPtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_proxy_from_daughter_eq_def2PtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated_bufferGlobal(N_bins_phi, 0.0);

    // For the error estimation on a per-candidate basis:
    std::vector<double> hRingObservable_daughter_per_candidate_squared_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_daughter_per_candidate_squared_PtYCuts_bufferGlobal(N_bins_phi, 0.0);
    std::vector<double> hRingObservable_daughter_per_candidate_squared_PtYCuts_integrated_bufferGlobal(1, 0.0);
    ...
    auto hLambdaCounter_phi_pT_Weighted = new TH2D("hLambdaCounter_phi_pT_Weighted", "hLambdaCounter_phi_pT_Weighted", N_bins_phi, phi_min, phi_max, N_bins_pT, pT_min, pT_max);
    auto hLambdaPolX_phi_pT_Weighted = new TH2D("hLambdaPolX_phi_pT_Weighted", "hLambdaPolX_phi_pT_Weighted", N_bins_phi, phi_min, phi_max, N_bins_pT, pT_min, pT_max);
    auto hLambdaPolY_phi_pT_Weighted = new TH2D("hLambdaPolY_phi_pT_Weighted", "hLambdaPolY_phi_pT_Weighted", N_bins_phi, phi_min, phi_max, N_bins_pT, pT_min, pT_max);
    auto hLambdaPolZ_phi_pT_Weighted = new TH2D("hLambdaPolZ_phi_pT_Weighted", "hLambdaPolZ_phi_pT_Weighted", N_bins_phi, phi_min, phi_max, N_bins_pT, pT_min, pT_max);
    ...
    auto hLambdaPolX_pT_y_phi_Weighted = new TH3D("hLambdaPolX_pT_y_phi_Weighted", "hLambdaPolX_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaPolY_pT_y_phi_Weighted = new TH3D("hLambdaPolY_pT_y_phi_Weighted", "hLambdaPolY_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaPolZ_pT_y_phi_Weighted = new TH3D("hLambdaPolZ_pT_y_phi_Weighted", "hLambdaPolZ_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    ...
    auto hLambdaCounter_pT_y_Weighted = new TH2D("hLambdaCounter_pT_y_Weighted", "hLambdaCounter_pT_y_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max); // Properly weighted counts
    ...
    auto hLambdaCounter_pT_y_phi_Weighted = new TH3D("hLambdaCounter_pT_y_phi_Weighted", "hLambdaCounter_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaAvgDotX_pT_y_phi_Weighted = new TH3D("hLambdaAvgDotX_pT_y_phi_Weighted", "hLambdaAvgDotX_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaAvgDotY_pT_y_phi_Weighted = new TH3D("hLambdaAvgDotY_pT_y_phi_Weighted", "hLambdaAvgDotY_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaAvgDotZ_pT_y_phi_Weighted = new TH3D("hLambdaAvgDotZ_pT_y_phi_Weighted", "hLambdaAvgDotZ_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);

            // For estimating the error of these averages
                // Notice that this error will be the same for all of these four equivalent quantities:
                    // - hRingObservable_proxy_from_daughter_eq_defPtYCuts
                    // - hRingObservable_proxy_from_daughter_star_eq_defPtYCuts
                    // - hRingObservableRecoPtYCuts
                    // - hRingObservableStarRecoPtYCuts
    auto hLambdaCounterSquared_pT_y_phi_Weighted = new TH3D("hLambdaCounterSquared_pT_y_phi_Weighted", "hLambdaCounterSquared_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaAvgDotXSquared_pT_y_phi_Weighted = new TH3D("hLambdaAvgDotXSquared_pT_y_phi_Weighted", "hLambdaAvgDotXSquared_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaAvgDotYSquared_pT_y_phi_Weighted = new TH3D("hLambdaAvgDotYSquared_pT_y_phi_Weighted", "hLambdaAvgDotYSquared_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaAvgDotZSquared_pT_y_phi_Weighted = new TH3D("hLambdaAvgDotZSquared_pT_y_phi_Weighted", "hLambdaAvgDotZSquared_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);

    auto hLambdaNeff_pT_y_phi_Weighted = new TH3D("hLambdaNeff_pT_y_phi_Weighted", "hLambdaNeff_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaNeff_minus_1_pT_y_phi_Weighted = new TH3D("hLambdaNeff_minus_1_pT_y_phi_Weighted", "hLambdaNeff_minus_1_pT_y_phi_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    auto hLambdaAvgDotX_pT_y_phi_Weighted_Err = new TH3D("hLambdaAvgDotX_pT_y_phi_Weighted_Err", "hLambdaAvgDotX_pT_y_phi_Weighted_Err", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);
    ...
    auto hRingObservableRecoPtYCuts_Err = new TH1D("hRingObservableRecoPtYCuts_Err", "hRingObservableRecoPtYCuts_Err", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservableReco_Err = new TH1D("hRingObservableReco_Err", "hRingObservableReco_Err", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservableRecoPtYCuts_integrated_Err = new TH1D("hRingObservableRecoPtYCuts_integrated_Err", "hRingObservableRecoPtYCuts_integrated_Err", 1, phi_min_Ring, phi_max_Ring);
    ...
    auto hLambdaCounter_phi_Weighted = new TH1D("hLambdaCounter_phi_Weighted", "hLambdaCounter_phi_Weighted", N_bins_phi, phi_min, phi_max);
    auto hLambdaCounter_phiRingAngles_Weighted = new TH1D("hLambdaCounter_phiRingAngles_Weighted", "hLambdaCounter_phiRingAngles_Weighted", N_bins_phi, phi_min_Ring, phi_max_Ring);
    // This counter here is equivalent to hLambdaCounter_phiRingAngles_Weighted because we do a coordinate transform that sets phi_random[ev_idx] = 0.
    ...
    auto hProtonCounter_phiRing_Weighted = new TH1D("hProtonCounter_phiRing_Weighted", "hProtonCounter_phiRing_Weighted", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hProtonStarCounter_phiRing_Weighted = new TH1D("hProtonStarCounter_phiRing_Weighted", "hProtonStarCounter_phiRing_Weighted", N_bins_phi, phi_min_Ring, phi_max_Ring);

            // Counters for the PtYCuts selection's normalization:
    auto hLambdaCounter_phi_Weighted_PtYCuts = new TH1D("hLambdaCounter_phi_Weighted_PtYCuts", "hLambdaCounter_phi_Weighted_PtYCuts", N_bins_phi, phi_min, phi_max);
    auto hLambdaCounter_phiRingAngles_Weighted_PtYCuts = new TH1D("hLambdaCounter_phiRingAngles_Weighted_PtYCuts", "hLambdaCounter_phiRingAngles_Weighted_PtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hProtonCounter_phiRing_Weighted_PtYCuts = new TH1D("hProtonCounter_phiRing_Weighted_PtYCuts", "hProtonCounter_phiRing_Weighted_PtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hProtonStarCounter_phiRing_Weighted_PtYCuts = new TH1D("hProtonStarCounter_phiRing_Weighted_PtYCuts", "hProtonStarCounter_phiRing_Weighted_PtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);


    auto hLambdaPolX_phi_Weighted = new TH1D("hLambdaPolX_phi_Weighted", "hLambdaPolX_phi_Weighted", N_bins_phi, phi_min, phi_max);
    auto hLambdaPolY_phi_Weighted = new TH1D("hLambdaPolY_phi_Weighted", "hLambdaPolY_phi_Weighted", N_bins_phi, phi_min, phi_max);
    auto hLambdaPolZ_phi_Weighted = new TH1D("hLambdaPolZ_phi_Weighted", "hLambdaPolZ_phi_Weighted", N_bins_phi, phi_min, phi_max);

        // Now for the reconstructions in 3D:
            // With the new CopyTH3D version, these have to be properly initialized objects!
    TH3D *hLambdaPolStarX_pT_y_phiReco_Weighted = new TH3D("hLambdaPolStarX_pT_y_phiReco_Weighted", "hLambdaPolStarX_pT_y_phiReco_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);;
    TH3D *hLambdaPolStarY_pT_y_phiReco_Weighted = new TH3D("hLambdaPolStarY_pT_y_phiReco_Weighted", "hLambdaPolStarY_pT_y_phiReco_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);;
    TH3D *hLambdaPolStarZ_pT_y_phiReco_Weighted = new TH3D("hLambdaPolStarZ_pT_y_phiReco_Weighted", "hLambdaPolStarZ_pT_y_phiReco_Weighted", N_bins_pT, pT_min, pT_max, N_bins_rap, -rap_max, rap_max, N_bins_phi, phi_min, phi_max);;
    ...
    auto hRingObservable_TrueValue = new TH1D("hRingObservable_TrueValue", "hRingObservable_TrueValue", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_TrueValuePtYCuts = new TH1D("hRingObservable_TrueValuePtYCuts", "hRingObservable_TrueValuePtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_NormPolTrueValuePtYCuts = new TH1D("hRingObservable_NormPolTrueValuePtYCuts", "hRingObservable_NormPolTrueValuePtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    ...
    TH1D *hRingObservableRecoPtYCuts = new TH1D("hRingObservableRecoPtYCuts", "hRingObservableRecoPtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    TH1D *hRingObservableNormPolRecoPtYCuts = new TH1D("hRingObservableNormPolRecoPtYCuts", "hRingObservableNormPolRecoPtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    TH1D *hRingObservableStarRecoPtYCuts = new TH1D("hRingObservableStarRecoPtYCuts", "hRingObservableStarRecoPtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    TH1D *hRingObservableStarNormPolRecoPtYCuts = new TH1D("hRingObservableStarNormPolRecoPtYCuts", "hRingObservableStarNormPolRecoPtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_proxy_from_daughter = new TH1D("hRingObservable_proxy_from_daughter", "hRingObservable_proxy_from_daughter", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_proxy_from_daughter_eq_def = new TH1D("hRingObservable_proxy_from_daughter_eq_def", "hRingObservable_proxy_from_daughter_eq_def", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_proxy_from_daughter_eq_defPtYCuts = new TH1D("hRingObservable_proxy_from_daughter_eq_defPtYCuts", "hRingObservable_proxy_from_daughter_eq_defPtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_proxy_from_daughter_eq_def2PtYCuts = new TH1D("hRingObservable_proxy_from_daughter_eq_def2PtYCuts", "hRingObservable_proxy_from_daughter_eq_def2PtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_proxy_from_daughter_star_eq_def = new TH1D("hRingObservable_proxy_from_daughter_star_eq_def", "hRingObservable_proxy_from_daughter_star_eq_def", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_proxy_from_daughter_star_eq_defPtYCuts = new TH1D("hRingObservable_proxy_from_daughter_star_eq_defPtYCuts", "hRingObservable_proxy_from_daughter_star_eq_defPtYCuts", N_bins_phi, phi_min_Ring, phi_max_Ring);

        // Integrated ring observable histograms:
    auto hRingObservable_TrueValuePtYCuts_integrated = new TH1D("hRingObservable_TrueValuePtYCuts_integrated", "hRingObservable_TrueValuePtYCuts_integrated", 1, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_NormPolTrueValuePtYCuts_integrated = new TH1D("hRingObservable_NormPolTrueValuePtYCuts_integrated", "hRingObservable_NormPolTrueValuePtYCuts_integrated", 1, phi_min_Ring, phi_max_Ring);
    TH1D *hRingObservableRecoPtYCuts_integrated = new TH1D("hRingObservableRecoPtYCuts_integrated", "hRingObservableRecoPtYCuts_integrated", 1, phi_min_Ring, phi_max_Ring);
    TH1D *hRingObservableNormPolRecoPtYCuts_integrated = new TH1D("hRingObservableNormPolRecoPtYCuts_integrated", "hRingObservableNormPolRecoPtYCuts_integrated", 1, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated = new TH1D("hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated", "hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated", 1, phi_min_Ring, phi_max_Ring);
    auto hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated = new TH1D("hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated", "hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated", 1, phi_min_Ring, phi_max_Ring);

    auto hDebugCounter_phi_star_sampler = new TH1D("hDebugCounter_phi_star_sampler", "hDebugCounter_phi_star_sampler", N_bins_phi, phi_min_Ring, phi_max_Ring);

    // Error propagation for the per-candidate observables (here we don't have true per candidate values, but the error bars may be estimated from this!):
    ...
    auto hRingObservable_daughter_per_candidate_PtYCuts_integrated_err = new TH1D("hRingObservable_daughter_per_candidate_PtYCuts_integrated_err", "hRingObservable_daughter_per_candidate_PtYCuts_integrated_err", 1, phi_min_Ring, phi_max_Ring);
        // Saving a variation of the Ring Observable that is essentially a copy of the previous versions, but with this smaller error:
    auto hRingObservableReco_with_err_per_candidate = new TH1D("hRingObservableReco_with_err_per_candidate", "hRingObservableReco_with_err_per_candidate", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservableRecoPtYCuts_with_err_per_candidate = new TH1D("hRingObservableRecoPtYCuts_with_err_per_candidate", "hRingObservableRecoPtYCuts_with_err_per_candidate", N_bins_phi, phi_min_Ring, phi_max_Ring);
    auto hRingObservableRecoPtYCuts_integrated_with_err_per_candidate = new TH1D("hRingObservableRecoPtYCuts_integrated_with_err_per_candidate", "hRingObservableRecoPtYCuts_integrated_with_err_per_candidate", 1, phi_min_Ring, phi_max_Ring);
    ...
    for (int ev_idx = 0; ev_idx < N_events; ev_idx++){
        ...
        for (int particle_idx = 0; particle_idx < mult_matrix[ev_idx].size(); particle_idx++){
            ...
            int global_bin_idx_pT_y_phi = hLambdaCounter_pT_y_phi_Weighted->FindBin(pT_matrix[ev_idx][particle_idx], y_matrix[ev_idx][particle_idx], phi_matrix[ev_idx][particle_idx]);
            int global_bin_idx_phi_pT = hLambdaCounter_phi_pT_Weighted->FindBin(phi_matrix[ev_idx][particle_idx], pT_matrix[ev_idx][particle_idx]);
            int global_bin_idx_pT_y = hLambdaCounter_pT_y_Weighted->FindBin(pT_matrix[ev_idx][particle_idx], y_matrix[ev_idx][particle_idx]);
            int global_bin_idx_phi = hLambdaCounter_phi_Weighted->FindBin(phi_matrix[ev_idx][particle_idx]);
            int global_bin_idx_phiRing = hLambdaCounter_phiRingAngles_Weighted->FindBin(wrapToInterval(phi_matrix[ev_idx][particle_idx], phi_min_Ring, phi_max_Ring)); // Be cautious with this! The global bin number will change in this coordinate system here!
            ...
            compact_bin_idx_pT_y_phi[ev_idx_times_N_bins_total + particle_idx] = mapGlobalToCompact(*(hLambdaCounter_pT_y_phi_Weighted), global_bin_idx_pT_y_phi);
            compact_bin_idx_phi_pT[ev_idx_times_N_bins_total + particle_idx] = mapGlobalToCompact(*(hLambdaCounter_phi_pT_Weighted), global_bin_idx_phi_pT);
            compact_bin_idx_pT_y[ev_idx_times_N_bins_total + particle_idx] = mapGlobalToCompact(*(hLambdaCounter_pT_y_Weighted), global_bin_idx_pT_y);
            compact_bin_idx_phi[ev_idx_times_N_bins_total + particle_idx] = mapGlobalToCompact(*(hLambdaCounter_phi_Weighted), global_bin_idx_phi);
            compact_bin_idx_phiRing[ev_idx_times_N_bins_total + particle_idx] = mapGlobalToCompact(*(hLambdaCounter_phiRingAngles_Weighted), global_bin_idx_phiRing);
    ...
    for (int resample_idx = 0; resample_idx < N_resamples; resample_idx++){
        ...
        // std::mt19937 rng(rd() + resample_idx); // Should actually use a different rng sequence for each iteration, not for each core! Otherwise, a same core might have the same rng for all iterations!
        // Redone randomizer of current core and iteration loop:
        std::seed_seq seed_seq{
        // Samples from time, the current loop index and the thread number, to guarantee randomization.
        ...
        std::vector<double> hLambdaCounterSquared_pT_y_phi_Weighted_buffer(N_bins_total, 0.0);
        std::vector<double> hLambdaAvgDotXSquared_pT_y_phi_Weighted_buffer(N_bins_total, 0.0);
        ...
        std::vector<double> hProtonCounter_phiRing_Weighted_buffer(N_bins_phi, 0.0);
        std::vector<double> hProtonStarCounter_phiRing_Weighted_buffer(N_bins_phi, 0.0);
        std::vector<double> hDebugCounter_phi_star_sampler_buffer(N_bins_phi, 0.0);

        std::vector<double> hLambdaCounter_phi_PtYCuts_Weighted_buffer(N_bins_phi, 0.0);
        std::vector<double> hLambdaCounter_phiRingAngles_PtYCuts_Weighted_buffer(N_bins_phi, 0.0);
        std::vector<double> hProtonCounter_phiRing_PtYCuts_Weighted_buffer(N_bins_phi, 0.0);
        std::vector<double> hProtonStarCounter_phiRing_PtYCuts_Weighted_buffer(N_bins_phi, 0.0);
        ...
        std::vector<double> hRingObservable_TrueValuePtYCuts_buffer(N_bins_phi, 0.0);
        std::vector<double> hRingObservable_NormPolTrueValuePtYCuts_buffer(N_bins_phi, 0.0);
        std::vector<double> hRingObservable_TrueValuePtYCuts_integrated_buffer(1, 0.0);
        std::vector<double> hRingObservable_NormPolTrueValuePtYCuts_integrated_buffer(1, 0.0);
        std::vector<double> hRingObservable_proxy_from_daughter_eq_defPtYCuts_buffer(N_bins_phi, 0.0);
        std::vector<double> hRingObservable_proxy_from_daughter_eq_def2PtYCuts_buffer(N_bins_phi, 0.0);
        std::vector<double> hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_buffer(N_bins_phi, 0.0);
        std::vector<double> hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated_buffer(1, 0.0);
        std::vector<double> hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated_buffer(1, 0.0);
        // std::cout << "DEBUG! Thread locals declared" << std::endl;

        // For the per-candidate error calculation:
        std::vector<double> hRingObservable_daughter_per_candidate_squared_buffer(N_bins_phi, 0.0);
        std::vector<double> hRingObservable_daughter_per_candidate_squared_PtYCuts_buffer(N_bins_phi, 0.0);
        std::vector<double> hRingObservable_daughter_per_candidate_squared_PtYCuts_integrated_buffer(1, 0.0);
        ...
        for (int ev_idx = 0; ev_idx < N_events; ev_idx++){
            ...
            for (int particle_idx = 0; particle_idx < y_matrix[ev_idx].size(); particle_idx++){ // Not truly a particle loop: the data is pre-binned, so this is a loop on bins of each event, which can be treated as "mean particles" of each (pT, phi, y) bin/interval
                ...
                // hLambdaCounter_DeltaphiJRing_Weighted.Get()->Fill(wrapToInterval(lambda_phi - phi_random[ev_idx], phi_min_Ring, phi_max_Ring), current_particle_multiplicity); // You actually need to have this counter defined as lambda_phi - phi_random[ev_idx] because the ring observable has a coordinate shift!
                    // This counter is equivalent to hLambdaCounter_phiRingAngles_Weighted_buffer because we are already assuming phi_random[ev_idx] = 0
                
                    // Determining the bin at which the buffer should be filled for these three counters:
                double proton_phi_wrapped = wrapToInterval(proton_4_momentum.Phi(), phi_min_Ring, phi_max_Ring);
                double proton_star_phi_wrapped = wrapToInterval(proton_4_momentum_star.Phi(), phi_min_Ring, phi_max_Ring); // The proton's phi in the laboratory's XYZ definition, not the local axis where the polarization points in Z'
                double phi_star_wrapped = wrapToInterval(phi_star, phi_min_Ring, phi_max_Ring); // This angle here is the proton's angle on the local coordinate axis where Z' points in the polarization direction!

                        // Now finding the bin to which those angles should belong to:
                int global_bin_idx_phiRing_proton_phi_wrapped = hLambdaCounter_phiRingAngles_Weighted->FindBin(proton_phi_wrapped);
                int global_bin_idx_phiRing_proton_star_phi_wrapped = hLambdaCounter_phiRingAngles_Weighted->FindBin(proton_star_phi_wrapped);
                int global_bin_idx_phiRing_phi_star_wrapped = hLambdaCounter_phiRingAngles_Weighted->FindBin(phi_star_wrapped);
                
                int compact_bin_idx_phiRing_proton_phi_wrapped_value = mapGlobalToCompact(*(hLambdaCounter_phiRingAngles_Weighted), global_bin_idx_phiRing_proton_phi_wrapped);
                int compact_bin_idx_phiRing_proton_star_phi_wrapped_value = mapGlobalToCompact(*(hLambdaCounter_phiRingAngles_Weighted), global_bin_idx_phiRing_proton_star_phi_wrapped);
                int compact_bin_idx_phiRing_phi_star_wrapped_value = mapGlobalToCompact(*(hLambdaCounter_phiRingAngles_Weighted), global_bin_idx_phiRing_phi_star_wrapped);

                hProtonCounter_phiRing_Weighted_buffer[compact_bin_idx_phiRing_proton_phi_wrapped_value] += current_particle_multiplicity; // A counter for the proton's angular distribution too, for the other ring estimator!
                hProtonStarCounter_phiRing_Weighted_buffer[compact_bin_idx_phiRing_proton_star_phi_wrapped_value] += current_particle_multiplicity; // For the other proxy of the ring observable, which should be normalized by the number of protons
                hDebugCounter_phi_star_sampler_buffer[compact_bin_idx_phiRing_phi_star_wrapped_value] += 1; // A debug histogram, just to know if the sampling is being truly random in phi_star, before coordinate rotation
                ...
                // delta_phi_J = wrapToInterval(delta_phi_J, phi_min_Ring, phi_max_Ring);
                    ...
                    // Calculating moments of the proton momentum distribution for the error estimation of these variables (in the current bin!):
                double counter_squared = current_particle_multiplicity*current_particle_multiplicity;
                // double square_norm_momentum_x_weighted = current_particle_multiplicity * proton_4_momentum.X()**2; // Will not estimate this error for now... not much to gain from it!
                ...
                // For the error estimation on a per-candidate basis:
                hRingObservable_daughter_per_candidate_squared_buffer[compact_bin_idx_phiRing_value] += RP_temp_proton_star_eqv_def*RP_temp_proton_star_eqv_def * current_particle_multiplicity;
                ...
                if ((lambda_pT > 0.5 && lambda_pT < 1.5) && (lambda_y > -0.5 && lambda_y < 0.5)){
                    hRingObservable_TrueValuePtYCuts_buffer[compact_bin_idx_phiRing_value] += RP_temp_true * current_particle_multiplicity;
                    hRingObservable_NormPolTrueValuePtYCuts_buffer[compact_bin_idx_phiRing_value] += RP_temp_true_Norm * current_particle_multiplicity;

                    hRingObservable_TrueValuePtYCuts_integrated_buffer[0] += RP_temp_true * current_particle_multiplicity;
                    hRingObservable_NormPolTrueValuePtYCuts_integrated_buffer[0] += RP_temp_true_Norm * current_particle_multiplicity;

                    hRingObservable_proxy_from_daughter_eq_defPtYCuts_buffer[compact_bin_idx_phiRing_value] += RP_temp_eqv_def * current_particle_multiplicity;
                    hRingObservable_proxy_from_daughter_eq_def2PtYCuts_buffer[compact_bin_idx_phiRing_value] += RP_temp_eqv_def2 * current_particle_multiplicity;
                    hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_buffer[compact_bin_idx_phiRing_value] += RP_temp_proton_star_eqv_def * current_particle_multiplicity;

                    hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated_buffer[0] += RP_temp_eqv_def * current_particle_multiplicity;
                    hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated_buffer[0] += RP_temp_proton_star_eqv_def * current_particle_multiplicity;

                    // Incrementing counters which are influenced by PtYCuts:
                    hLambdaCounter_phi_PtYCuts_Weighted_buffer[compact_bin_idx_phi_value] += current_particle_multiplicity;
                    hLambdaCounter_phiRingAngles_PtYCuts_Weighted_buffer[compact_bin_idx_phiRing_value] += current_particle_multiplicity;
                    hProtonCounter_phiRing_PtYCuts_Weighted_buffer[compact_bin_idx_phiRing_proton_phi_wrapped_value] += current_particle_multiplicity;
                    hProtonStarCounter_phiRing_PtYCuts_Weighted_buffer[compact_bin_idx_phiRing_proton_star_phi_wrapped_value] += current_particle_multiplicity;

                    // For the error estimation on a per-candidate basis:
                    hRingObservable_daughter_per_candidate_squared_PtYCuts_buffer[compact_bin_idx_phiRing_value] += RP_temp_proton_star_eqv_def*RP_temp_proton_star_eqv_def * current_particle_multiplicity;
                    hRingObservable_daughter_per_candidate_squared_PtYCuts_integrated_buffer[0] += RP_temp_proton_star_eqv_def*RP_temp_proton_star_eqv_def * current_particle_multiplicity;
                }
        ...
        // Emptying thread buffers after all events where processed -- No true gain in doing this histogram filling on a per-event basis...
        ...
        #pragma omp critical
        {
            // Doing the flush in a sequential manner (omp critical), so that there is no chance of multiple access to same object, and guaranteeing thread safety
            ...
            // Will divide the flushing in 4 categories, one for each buffer size:
                // 1 - Flushing 3D buffers into global buffer:
            for (int CompactBinIndex_pT_y_phi = 0; CompactBinIndex_pT_y_phi < N_bins_total; CompactBinIndex_pT_y_phi++){
                hLambdaCounter_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi] += hLambdaCounter_pT_y_phi_Weighted_buffer[CompactBinIndex_pT_y_phi];
                hLambdaAvgDotX_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi] += hLambdaAvgDotX_pT_y_phi_Weighted_buffer[CompactBinIndex_pT_y_phi];
                hLambdaAvgDotY_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi] += hLambdaAvgDotY_pT_y_phi_Weighted_buffer[CompactBinIndex_pT_y_phi];
                hLambdaAvgDotZ_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi] += hLambdaAvgDotZ_pT_y_phi_Weighted_buffer[CompactBinIndex_pT_y_phi];

                hLambdaCounterSquared_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi] += hLambdaCounterSquared_pT_y_phi_Weighted_buffer[CompactBinIndex_pT_y_phi];
                ...
                hLambdaPolZ_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi] += hLambdaPolZ_pT_y_phi_Weighted_buffer[CompactBinIndex_pT_y_phi];
            }
            ...
            for (int CompactBinIndex_pT_y = 0; CompactBinIndex_pT_y < N_bins_pT*N_bins_rap; CompactBinIndex_pT_y++){
                hLambdaCounter_pT_y_Weighted_bufferGlobal[CompactBinIndex_pT_y] += hLambdaCounter_pT_y_Weighted_buffer[CompactBinIndex_pT_y];
            }
            ...
            for (int CompactBinIndex_phi_pT = 0; CompactBinIndex_phi_pT < N_bins_phi*N_bins_pT; CompactBinIndex_phi_pT++){
                hLambdaPolX_phi_pT_Weighted_bufferGlobal[CompactBinIndex_phi_pT] += hLambdaPolX_phi_pT_Weighted_buffer[CompactBinIndex_phi_pT];
                hLambdaPolY_phi_pT_Weighted_bufferGlobal[CompactBinIndex_phi_pT] += hLambdaPolY_phi_pT_Weighted_buffer[CompactBinIndex_phi_pT];
                hLambdaPolZ_phi_pT_Weighted_bufferGlobal[CompactBinIndex_phi_pT] += hLambdaPolZ_phi_pT_Weighted_buffer[CompactBinIndex_phi_pT];
                hLambdaCounter_phi_pT_Weighted_bufferGlobal[CompactBinIndex_phi_pT] += hLambdaCounter_phi_pT_Weighted_buffer[CompactBinIndex_phi_pT];
            }
            ...
            for (int CompactBinIndex_phi = 0; CompactBinIndex_phi < N_bins_phi; CompactBinIndex_phi++){
                hLambdaCounter_phi_Weighted_bufferGlobal[CompactBinIndex_phi] += hLambdaCounter_phi_Weighted_buffer[CompactBinIndex_phi];
                hLambdaCounter_phiRingAngles_Weighted_bufferGlobal[CompactBinIndex_phi] += hLambdaCounter_phiRingAngles_Weighted_buffer[CompactBinIndex_phi];
                hProtonCounter_phiRing_Weighted_bufferGlobal[CompactBinIndex_phi] += hProtonCounter_phiRing_Weighted_buffer[CompactBinIndex_phi];
                hProtonStarCounter_phiRing_Weighted_bufferGlobal[CompactBinIndex_phi] += hProtonStarCounter_phiRing_Weighted_buffer[CompactBinIndex_phi];
                hDebugCounter_phi_star_sampler_bufferGlobal[CompactBinIndex_phi] += hDebugCounter_phi_star_sampler_buffer[CompactBinIndex_phi];
                
                hLambdaPolX_phi_Weighted_bufferGlobal[CompactBinIndex_phi] += hLambdaPolX_phi_Weighted_buffer[CompactBinIndex_phi];
                hLambdaPolY_phi_Weighted_bufferGlobal[CompactBinIndex_phi] += hLambdaPolY_phi_Weighted_buffer[CompactBinIndex_phi];
                hLambdaPolZ_phi_Weighted_bufferGlobal[CompactBinIndex_phi] += hLambdaPolZ_phi_Weighted_buffer[CompactBinIndex_phi];

                hRingObservable_proxy_from_daughter_bufferGlobal[CompactBinIndex_phi] += hRingObservable_proxy_from_daughter_buffer[CompactBinIndex_phi];
                hRingObservable_proxy_from_daughter_eq_def_bufferGlobal[CompactBinIndex_phi] += hRingObservable_proxy_from_daughter_eq_def_buffer[CompactBinIndex_phi];
                hRingObservable_proxy_from_daughter_star_eq_def_bufferGlobal[CompactBinIndex_phi] += hRingObservable_proxy_from_daughter_star_eq_def_buffer[CompactBinIndex_phi];
                hRingObservable_TrueValue_bufferGlobal[CompactBinIndex_phi] += hRingObservable_TrueValue_buffer[CompactBinIndex_phi];
                hRingObservable_TrueValuePtYCuts_bufferGlobal[CompactBinIndex_phi] += hRingObservable_TrueValuePtYCuts_buffer[CompactBinIndex_phi];
                hRingObservable_NormPolTrueValuePtYCuts_bufferGlobal[CompactBinIndex_phi] += hRingObservable_NormPolTrueValuePtYCuts_buffer[CompactBinIndex_phi];

                hRingObservable_proxy_from_daughter_eq_defPtYCuts_bufferGlobal[CompactBinIndex_phi] += hRingObservable_proxy_from_daughter_eq_defPtYCuts_buffer[CompactBinIndex_phi];
                hRingObservable_proxy_from_daughter_eq_def2PtYCuts_bufferGlobal[CompactBinIndex_phi] += hRingObservable_proxy_from_daughter_eq_def2PtYCuts_buffer[CompactBinIndex_phi];
                hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_bufferGlobal[CompactBinIndex_phi] += hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_buffer[CompactBinIndex_phi];

                // For counters that were influenced by PtYCuts:
                hLambdaCounter_phi_Weighted_PtYCuts_bufferGlobal[CompactBinIndex_phi] += hLambdaCounter_phi_PtYCuts_Weighted_buffer[CompactBinIndex_phi];
                hLambdaCounter_phiRingAngles_Weighted_PtYCuts_bufferGlobal[CompactBinIndex_phi] += hLambdaCounter_phiRingAngles_PtYCuts_Weighted_buffer[CompactBinIndex_phi];
                hProtonCounter_phiRing_Weighted_PtYCuts_bufferGlobal[CompactBinIndex_phi] += hProtonCounter_phiRing_PtYCuts_Weighted_buffer[CompactBinIndex_phi];
                hProtonStarCounter_phiRing_Weighted_PtYCuts_bufferGlobal[CompactBinIndex_phi] += hProtonStarCounter_phiRing_PtYCuts_Weighted_buffer[CompactBinIndex_phi];

                // For the error estimation on a per-candidate basis:
                hRingObservable_daughter_per_candidate_squared_bufferGlobal[CompactBinIndex_phi] += hRingObservable_daughter_per_candidate_squared_buffer[CompactBinIndex_phi];
                hRingObservable_daughter_per_candidate_squared_PtYCuts_bufferGlobal[CompactBinIndex_phi] += hRingObservable_daughter_per_candidate_squared_PtYCuts_buffer[CompactBinIndex_phi];
            }
                // Filling integrated buffers (0D):
            hRingObservable_TrueValuePtYCuts_integrated_bufferGlobal[0] += hRingObservable_TrueValuePtYCuts_integrated_buffer[0];
            hRingObservable_NormPolTrueValuePtYCuts_integrated_bufferGlobal[0] += hRingObservable_NormPolTrueValuePtYCuts_integrated_buffer[0];
            hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated_bufferGlobal[0] += hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated_buffer[0];
            hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated_bufferGlobal[0] += hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated_buffer[0];
            hRingObservable_daughter_per_candidate_squared_PtYCuts_integrated_bufferGlobal[0] += hRingObservable_daughter_per_candidate_squared_PtYCuts_integrated_buffer[0];
    ...
    std::cout << "\tDone resampling!" << std::endl;
        ...
        // Calculating resampler elapsed time:
    auto resample_end_time = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration_resample = resample_end_time - start_time - duration_fetch - pre_process_duration;
    std::cout << "Spent " << duration_resample.count() << " seconds resampling data." << std::endl;
    ...
    ////////////////////////////////////////////////////////////////////
    // Flushing the C++ vector buffers into the histogram objects:
    ////////////////////////////////////////////////////////////////////
    std::cout << "\nFlushing buffers into histograms" << std::endl;
        // Will divide the flushing in 4 categories, one for each buffer size:
    ...
    for (int CompactBinIndex_pT_y_phi = 0; CompactBinIndex_pT_y_phi < N_bins_total; CompactBinIndex_pT_y_phi++){
        ...
        int GlobalBinIndex_pT_y_phi = mapCompactToGlobal(*(hLambdaCounter_pT_y_phi_Weighted), CompactBinIndex_pT_y_phi);
                // Useful debugging sequence format:
        // SAFE_ADDBINCONTENT(hLambdaCounter_pT_y_phi_Weighted, GlobalBinIndex_pT_y_phi, hLambdaCounter_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi]);
        hLambdaCounter_pT_y_phi_Weighted->AddBinContent(GlobalBinIndex_pT_y_phi, hLambdaCounter_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi]);
        hLambdaAvgDotX_pT_y_phi_Weighted->AddBinContent(GlobalBinIndex_pT_y_phi, hLambdaAvgDotX_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi]);
        hLambdaAvgDotY_pT_y_phi_Weighted->AddBinContent(GlobalBinIndex_pT_y_phi, hLambdaAvgDotY_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi]);
        hLambdaAvgDotZ_pT_y_phi_Weighted->AddBinContent(GlobalBinIndex_pT_y_phi, hLambdaAvgDotZ_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi]);

        hLambdaCounterSquared_pT_y_phi_Weighted->AddBinContent(GlobalBinIndex_pT_y_phi, hLambdaCounterSquared_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi]);
        hLambdaAvgDotXSquared_pT_y_phi_Weighted->AddBinContent(GlobalBinIndex_pT_y_phi, hLambdaAvgDotXSquared_pT_y_phi_Weighted_bufferGlobal[CompactBinIndex_pT_y_phi]);
    ...
    for (int CompactBinIndex_pT_y = 0; CompactBinIndex_pT_y < N_bins_pT*N_bins_rap; CompactBinIndex_pT_y++){
        // Converting the CompactBinIndex back to a global index:
        int GlobalBinIndex_pT_y = mapCompactToGlobal(*(hLambdaCounter_pT_y_Weighted), CompactBinIndex_pT_y);
        hLambdaCounter_pT_y_Weighted->AddBinContent(GlobalBinIndex_pT_y, hLambdaCounter_pT_y_Weighted_bufferGlobal[CompactBinIndex_pT_y]);
    }

        // 3 - Flushing 2D buffers for histograms with phi and pT binning:
    for (int CompactBinIndex_phi_pT = 0; CompactBinIndex_phi_pT < N_bins_phi*N_bins_pT; CompactBinIndex_phi_pT++){
        int GlobalBinIndex_phi_pT = mapCompactToGlobal(*(hLambdaPolX_phi_pT_Weighted), CompactBinIndex_phi_pT);
        hLambdaPolX_phi_pT_Weighted->AddBinContent(GlobalBinIndex_phi_pT, hLambdaPolX_phi_pT_Weighted_bufferGlobal[CompactBinIndex_phi_pT]);
        hLambdaPolY_phi_pT_Weighted->AddBinContent(GlobalBinIndex_phi_pT, hLambdaPolY_phi_pT_Weighted_bufferGlobal[CompactBinIndex_phi_pT]);
        hLambdaPolZ_phi_pT_Weighted->AddBinContent(GlobalBinIndex_phi_pT, hLambdaPolZ_phi_pT_Weighted_bufferGlobal[CompactBinIndex_phi_pT]);
        hLambdaCounter_phi_pT_Weighted->AddBinContent(GlobalBinIndex_phi_pT, hLambdaCounter_phi_pT_Weighted_bufferGlobal[CompactBinIndex_phi_pT]);
    }

        // 4 - Flushing 1D buffers of histograms in phi:
    for (int CompactBinIndex_phi = 0; CompactBinIndex_phi < N_bins_phi; CompactBinIndex_phi++){
        // Converting the CompactBinIndex'es back to global indexes:
        int GlobalBinIndex_phi = mapCompactToGlobal(*(hLambdaCounter_phi_Weighted), CompactBinIndex_phi);
        int GlobalBinIndex_phiRingAngles = mapCompactToGlobal(*(hLambdaCounter_phiRingAngles_Weighted), CompactBinIndex_phi);

        hLambdaCounter_phi_Weighted->AddBinContent(GlobalBinIndex_phi, hLambdaCounter_phi_Weighted_bufferGlobal[CompactBinIndex_phi]);
        hLambdaCounter_phiRingAngles_Weighted->AddBinContent(GlobalBinIndex_phiRingAngles, hLambdaCounter_phiRingAngles_Weighted_bufferGlobal[CompactBinIndex_phi]);
        hProtonCounter_phiRing_Weighted->AddBinContent(GlobalBinIndex_phiRingAngles, hProtonCounter_phiRing_Weighted_bufferGlobal[CompactBinIndex_phi]);
        hProtonStarCounter_phiRing_Weighted->AddBinContent(GlobalBinIndex_phiRingAngles, hProtonStarCounter_phiRing_Weighted_bufferGlobal[CompactBinIndex_phi]);
        hDebugCounter_phi_star_sampler->AddBinContent(GlobalBinIndex_phiRingAngles, hDebugCounter_phi_star_sampler_bufferGlobal[CompactBinIndex_phi]);

        hLambdaPolX_phi_Weighted->AddBinContent(GlobalBinIndex_phi, hLambdaPolX_phi_Weighted_bufferGlobal[CompactBinIndex_phi]);
        hLambdaPolY_phi_Weighted->AddBinContent(GlobalBinIndex_phi, hLambdaPolY_phi_Weighted_bufferGlobal[CompactBinIndex_phi]);
        hLambdaPolZ_phi_Weighted->AddBinContent(GlobalBinIndex_phi, hLambdaPolZ_phi_Weighted_bufferGlobal[CompactBinIndex_phi]);

        hRingObservable_proxy_from_daughter->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_proxy_from_daughter_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_proxy_from_daughter_eq_def->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_proxy_from_daughter_eq_def_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_proxy_from_daughter_star_eq_def->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_proxy_from_daughter_star_eq_def_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_TrueValue->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_TrueValue_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_TrueValuePtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_TrueValuePtYCuts_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_NormPolTrueValuePtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_NormPolTrueValuePtYCuts_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_proxy_from_daughter_eq_defPtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_proxy_from_daughter_eq_defPtYCuts_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_proxy_from_daughter_eq_def2PtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_proxy_from_daughter_eq_def2PtYCuts_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_proxy_from_daughter_star_eq_defPtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_bufferGlobal[CompactBinIndex_phi]);

        // Flushing counters that were affected by PtYCuts:
        hLambdaCounter_phi_Weighted_PtYCuts->AddBinContent(GlobalBinIndex_phi, hLambdaCounter_phi_Weighted_PtYCuts_bufferGlobal[CompactBinIndex_phi]);
        hLambdaCounter_phiRingAngles_Weighted_PtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hLambdaCounter_phiRingAngles_Weighted_PtYCuts_bufferGlobal[CompactBinIndex_phi]);
        hProtonCounter_phiRing_Weighted_PtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hProtonCounter_phiRing_Weighted_PtYCuts_bufferGlobal[CompactBinIndex_phi]);
        hProtonStarCounter_phiRing_Weighted_PtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hProtonStarCounter_phiRing_Weighted_PtYCuts_bufferGlobal[CompactBinIndex_phi]);

        // For the error estimation on a per-candidate basis:
        hRingObservable_daughter_per_candidate_squared->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_daughter_per_candidate_squared_bufferGlobal[CompactBinIndex_phi]);
        hRingObservable_daughter_per_candidate_squared_PtYCuts->AddBinContent(GlobalBinIndex_phiRingAngles, hRingObservable_daughter_per_candidate_squared_PtYCuts_bufferGlobal[CompactBinIndex_phi]);
    }

        // Filling integrated value histograms -- Just adding a value in 0. is enough, if done with the proper weight:
    hRingObservable_TrueValuePtYCuts_integrated->Fill(0., hRingObservable_TrueValuePtYCuts_integrated_bufferGlobal[0]);
    hRingObservable_NormPolTrueValuePtYCuts_integrated->Fill(0., hRingObservable_NormPolTrueValuePtYCuts_integrated_bufferGlobal[0]);
    hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated->Fill(0., hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated_bufferGlobal[0]);
    hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated->Fill(0., hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated_bufferGlobal[0]);
    hRingObservable_daughter_per_candidate_squared_PtYCuts_integrated->Fill(0., hRingObservable_daughter_per_candidate_squared_PtYCuts_integrated_bufferGlobal[0]);
    ...
    hLambdaAvgDotX_pT_y_phi_Weighted->Divide(hLambdaCounter_pT_y_phi_Weighted); // Do notice that in the new format, without .get(), we need to pass the address of the TH3D so that ROOT has the expected pointer
    ...
    hLambdaAvgDotXSquared_pT_y_phi_Weighted->Divide(hLambdaCounter_pT_y_phi_Weighted);
    ...
    CopyTH3D(hLambdaAvgDotZ_pT_y_phi_Weighted, hLambdaPolStarZ_pT_y_phiReco_Weighted);
    ...
    CopyTH3D(hLambdaAvgDotZ_pT_y_phi_Weighted, hLambdaAvgDotZ_pT_y_phi_Weighted_Err);

            // Now taking the square to get \mu_w^2:
    hLambdaAvgDotX_pT_y_phi_Weighted_Err->Multiply(hLambdaAvgDotX_pT_y_phi_Weighted_Err);
    hLambdaAvgDotY_pT_y_phi_Weighted_Err->Multiply(hLambdaAvgDotY_pT_y_phi_Weighted_Err);
    hLambdaAvgDotZ_pT_y_phi_Weighted_Err->Multiply(hLambdaAvgDotZ_pT_y_phi_Weighted_Err);

            // Now replacing the contents with <p_i^2>_w - \mu_w^2
    hLambdaAvgDotX_pT_y_phi_Weighted_Err->Add(hLambdaAvgDotXSquared_pT_y_phi_Weighted, hLambdaAvgDotX_pT_y_phi_Weighted_Err, 1., -1.);
    hLambdaAvgDotY_pT_y_phi_Weighted_Err->Add(hLambdaAvgDotYSquared_pT_y_phi_Weighted, hLambdaAvgDotY_pT_y_phi_Weighted_Err, 1., -1.);
    hLambdaAvgDotZ_pT_y_phi_Weighted_Err->Add(hLambdaAvgDotZSquared_pT_y_phi_Weighted, hLambdaAvgDotZ_pT_y_phi_Weighted_Err, 1., -1.);

            // Calculating the effective sample size (N_{eff} = hLambdaCounter_pT_y_phi_Weighted^2/hLambdaCounterSquared_pT_y_phi_Weighted)
    CopyTH3D(hLambdaCounter_pT_y_phi_Weighted, hLambdaNeff_pT_y_phi_Weighted);
    hLambdaNeff_pT_y_phi_Weighted->Multiply(hLambdaNeff_pT_y_phi_Weighted);
    ...
    hLambdaAvgDotX_pT_y_phi_Weighted_Err->Scale(1./(N_events*N_resamples-1)); // Using N_resamples-1 as an unbiased error estimator for low resampling numbers.
    hLambdaAvgDotY_pT_y_phi_Weighted_Err->Scale(1./(N_events*N_resamples-1));
    hLambdaAvgDotZ_pT_y_phi_Weighted_Err->Scale(1./(N_events*N_resamples-1));
    ...
    SqrtHist(hLambdaAvgDotZ_pT_y_phi_Weighted_Err);

            // Scaling these errors to get the error on polarization:
    CopyTH3D(hLambdaAvgDotX_pT_y_phi_Weighted_Err, hLambdaPolStarX_pT_y_phiReco_Weighted_Err);
    CopyTH3D(hLambdaAvgDotY_pT_y_phi_Weighted_Err, hLambdaPolStarY_pT_y_phiReco_Weighted_Err);
    CopyTH3D(hLambdaAvgDotZ_pT_y_phi_Weighted_Err, hLambdaPolStarZ_pT_y_phiReco_Weighted_Err);

    hLambdaPolStarX_pT_y_phiReco_Weighted_Err->Scale(3.0 / alpha_H);
    hLambdaPolStarY_pT_y_phiReco_Weighted_Err->Scale(3.0 / alpha_H);
    hLambdaPolStarZ_pT_y_phiReco_Weighted_Err->Scale(3.0 / alpha_H);
    ...
    // double total_Lambdas_PtYCuts = hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral();
    for (int pT_idx = 1; pT_idx <= N_bins_pT; pT_idx++){ // bin index starts at 1 (not 0)
        for (int y_idx = 1; y_idx <= N_bins_rap; y_idx++){
            for (int phi_idx = 1; phi_idx <= N_bins_phi; phi_idx++){
                ...
                //     // Introducing a variation that includes PtYCuts weights only for the ring observable:
                // double current_phiRing_bin_multiplicity_PtYCuts = hLambdaCounter_phiRingAngles_Weighted_PtYCuts->GetBinContent(phiRing_idx);
                ...
                double RP_temp_Norm = RP_temp/Pol_norm;

                    // Testing an alternative observable that uses the polarization on the rest frame instead of having to boost it:
                double RP_temp_star = ((P_Lambda_star_mean_4vec_Weighted.X()*cross_x + P_Lambda_star_mean_4vec_Weighted.Y()*cross_y + P_Lambda_star_mean_4vec_Weighted.Z()*cross_z)/
                                        cross_product_norm);
                double Pol_norm_star = std::sqrt(P_Lambda_star_mean_4vec_Weighted.X()*P_Lambda_star_mean_4vec_Weighted.X()
                    + P_Lambda_star_mean_4vec_Weighted.Y()*P_Lambda_star_mean_4vec_Weighted.Y()
                ...
                // Now for the PtYCuts variant of error calculation:
                double current_RingObservableReco_Err_squared_PtYCuts = (err_x*err_x + err_y*err_y + err_z*err_z)/(cross_product_norm*cross_product_norm) 
                                                                 * current_pTyphi_bin_multiplicity; // w_i
                                                                // Just for testing, placed this normalization outside the loop:
                                                                //  /(current_phi_bin_multiplicity*current_phi_bin_multiplicity); // 1/W^2
                double current_RingObservableReco_integrated_Err_squared_PtYCuts = (err_x*err_x + err_y*err_y + err_z*err_z)/(cross_product_norm*cross_product_norm) 
                                                                 * current_pTyphi_bin_multiplicity; // w_i
                                                                // Just for testing, placed this normalization outside the loop:
                                                                //  /(total_Lambdas_PtYCuts*total_Lambdas_PtYCuts); // 1/W^2 --> And here this corresponds to the total amount of Lambdas because we integrate along all dimensions!

                if ((pT_bin_center > 0.5 && pT_bin_center < 1.5) && (y_bin_center > -0.5 && y_bin_center < 0.5)){ // Corrected to introduce the rapidity cuts too!
                    hRingObservableRecoPtYCuts->Fill(delta_phi_J, RP_temp * current_pTyphi_bin_multiplicity);
                    hRingObservableNormPolRecoPtYCuts->Fill(delta_phi_J, RP_temp_Norm * current_pTyphi_bin_multiplicity);

                    hRingObservableStarRecoPtYCuts->Fill(delta_phi_J, RP_temp_star * current_pTyphi_bin_multiplicity);
                    hRingObservableStarNormPolRecoPtYCuts->Fill(delta_phi_J, RP_temp_star_Norm * current_pTyphi_bin_multiplicity);

                    hRingObservableRecoPtYCuts_integrated->Fill(delta_phi_J, RP_temp * current_pTyphi_bin_multiplicity);
                    hRingObservableNormPolRecoPtYCuts_integrated->Fill(delta_phi_J, RP_temp_Norm * current_pTyphi_bin_multiplicity);

                        // Now for the PtYCuts variation of error:
                    hRingObservableRecoPtYCuts_Err->Fill(delta_phi_J, current_RingObservableReco_Err_squared_PtYCuts); // This will need to receive a sqrt() later, after the averaging!
                    hRingObservableRecoPtYCuts_integrated_Err->Fill(delta_phi_J, current_RingObservableReco_integrated_Err_squared_PtYCuts);
                    ...
                    // std::cout << "\nMultiplicity of the current bin is: current_phiRing_bin_multiplicity_PtYCuts = " << current_phiRing_bin_multiplicity_PtYCuts << std::endl;
                    // std::cout << "Confirming, the expect bin center value is: " << delta_phi_J << " while the value we are accessing the multiplicity of is: " << hLambdaCounter_phiRingAngles_Weighted_PtYCuts->GetBinCenter(phiRing_idx) << std::endl;
                    // std::cout << "The total error^2 contribution here, already divided by the multiplicity of the current phi bin is: " << current_RingObservableReco_Err_squared_PtYCuts/current_phiRing_bin_multiplicity_PtYCuts << std::endl;
                    // std::cout << "And the square root of that is: " << std::sqrt(current_RingObservableReco_Err_squared_PtYCuts/current_phiRing_bin_multiplicity_PtYCuts) << std::endl;
                    // std::cout << "The current contribution to the ring observable would be: " << RP_temp_star * current_pTyphi_bin_multiplicity << std::endl;
                    // std::cout << "Or if you normalize that by the multiplicity of the current bin (compare with the sqrt of the error above): " << RP_temp_star * current_pTyphi_bin_multiplicity/current_phiRing_bin_multiplicity_PtYCuts << std::endl;

                    // if (current_RingObservableReco_Err_squared_PtYCuts < 0.){throw std::out_of_range("Error is smaller than 0 !!!");}
                }
            }
        }
    }
    ...
    // std::cout << "DEBUG! The last bin's error before the square root is: " << hRingObservableRecoPtYCuts_Err->GetBinContent(N_bins_phi) << std::endl;
    // std::cout << "DEBUG! The integrated bin's error before the square root is: " << hRingObservableRecoPtYCuts_integrated_Err->GetBinContent(1) << std::endl;
    
    SqrtHist(hRingObservableReco_Err);
    SqrtHist(hRingObservableRecoPtYCuts_Err);
    SqrtHist(hRingObservableRecoPtYCuts_integrated_Err);

    // std::cout << "DEBUG! The last bin's error after the square root is: " << hRingObservableRecoPtYCuts_Err->GetBinContent(N_bins_phi) << std::endl;
    // std::cout << "DEBUG! The integrated bin's error after the square root is: " << hRingObservableRecoPtYCuts_integrated_Err->GetBinContent(1) << std::endl;
    ...
    // for (int i = 1; i <= hRingObservableRecoPtYCuts_Err->GetNbinsX(); i++){
    //     double err_i = hRingObservableRecoPtYCuts_Err->GetBinContent(i);
    ...
    // std::cout << "DEBUG! The integral of hLambdaCounter_phiRingAngles_Weighted_PtYCuts is " << hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral() << std::endl;
    // // std::cout << "DEBUG! The integral of hRingObservableStarRecoPtYCuts (normalized) is " << hRingObservableStarRecoPtYCuts->Integral()/hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral() << std::endl;
    ...
    // // std::cout << "total_Lambdas_PtYCuts: "  << total_Lambdas_PtYCuts << std::endl; // They match with N_entries on the Counter histograms.
    // std::cout << "DEBUG! The integral of hRingObservableStarRecoPtYCuts (normalized by the Divide() method) is " << hRingObservableStarRecoPtYCuts->Integral() << std::endl;
    // std::cout << "DEBUG! The integral of its errors matrix is: " << hRingObservableRecoPtYCuts_Err->Integral() << std::endl;
    ...
    hRingObservable_TrueValue->Divide(hLambdaCounter_phiRingAngles_Weighted); // Notice I'm using hLambdaCounter_phiRingAngles_Weighted, not hLambdaCounter_DeltaphiJRing_Weighted_merged, because the DeltaPhiJ coordinates are equivalent to the Phi coordinates in this system where phi_random[ev_idx] is zero for all events!
    hRingObservable_TrueValuePtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts); // Using the PtYCuts counter to properly average only on the bins which are involved in the actual signal (the difference should be small because most of the counts are in the PtYCuts region though)
    hRingObservable_NormPolTrueValuePtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);
    hRingObservable_TrueValuePtYCuts_integrated->Scale(1./hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral());
    hRingObservable_NormPolTrueValuePtYCuts_integrated->Scale(1./hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral());

    hRingObservableReco->Divide(hLambdaCounter_phiRingAngles_Weighted);
    hRingObservableRecoPtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);
    hRingObservableNormPolRecoPtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);

    hRingObservableStarRecoPtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);
    hRingObservableStarNormPolRecoPtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);

    hRingObservableRecoPtYCuts_integrated->Scale(1./hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral());
    hRingObservableNormPolRecoPtYCuts_integrated->Scale(1./hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral());
    ...
    hRingObservable_proxy_from_daughter->Divide(hLambdaCounter_phiRingAngles_Weighted);
    hRingObservable_proxy_from_daughter_eq_def->Divide(hLambdaCounter_phiRingAngles_Weighted);
    hRingObservable_proxy_from_daughter_eq_defPtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);
    hRingObservable_proxy_from_daughter_eq_def2PtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);

    hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated->Scale(1./hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral());
    ...
    hRingObservable_proxy_from_daughter_star_eq_defPtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);
    hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated->Scale(1./hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral());

    // For the error estimation on a per-candidate basis:
    hRingObservable_daughter_per_candidate_squared->Divide(hLambdaCounter_phiRingAngles_Weighted);
    hRingObservable_daughter_per_candidate_squared_PtYCuts->Divide(hLambdaCounter_phiRingAngles_Weighted_PtYCuts);
    hRingObservable_daughter_per_candidate_squared_PtYCuts_integrated->Scale(1./hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Integral());
    ///////////////////////////////////////////////////////////////
    ...
    CopyTH1D(hRingObservable_proxy_from_daughter_star_eq_def, hRingObservable_daughter_per_candidate_err);
    CopyTH1D(hRingObservable_proxy_from_daughter_star_eq_defPtYCuts, hRingObservable_daughter_per_candidate_PtYCuts_err);
    CopyTH1D(hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated, hRingObservable_daughter_per_candidate_PtYCuts_integrated_err);
    // Now taking the square to get \mu^2:
    ...
    // CopyTH1D(hLambdaCounter_phiRingAngles_Weighted, hLambdaCounter_DeltaPhiJRingAngles_minus1); // Necessary for an unbiased error estimator for low-statistics
    // CopyTH1D(hLambdaCounter_phiRingAngles_Weighted_PtYCuts, hLambdaCounter_DeltaPhiJRingAngles_PtYCuts_minus1);
    //
    ...
    double n_resamples_integrated_with_cuts = N_events*N_resamples*(N_bins_pT_in_interval*N_bins_rap*N_bins_phi);
    hRingObservable_daughter_per_candidate_err->Scale(1./(n_resamples_per_phi_bin_no_cuts-1));
    hRingObservable_daughter_per_candidate_PtYCuts_err->Scale(1./(n_resamples_per_phi_bin_with_cuts-1));
    hRingObservable_daughter_per_candidate_PtYCuts_integrated_err->Scale(1./(n_resamples_integrated_with_cuts-1));

    // std::cout << "n_resamples_per_phi_bin_no_cuts: " << n_resamples_per_phi_bin_no_cuts << std::endl;
    // std::cout << "n_resamples_per_phi_bin_with_cuts: " << n_resamples_per_phi_bin_with_cuts << std::endl;
    // std::cout << "n_resamples_integrated_with_cuts: " << n_resamples_integrated_with_cuts << std::endl;
    // std::cout << "hRingObservable_daughter_per_candidate_PtYCuts_integrated_err->GetBinContent(1): " << hRingObservable_daughter_per_candidate_PtYCuts_integrated_err->GetBinContent(1) << std::endl;
    ...
    // std::cout << "After square root hRingObservable_daughter_per_candidate_PtYCuts_integrated_err->GetBinContent(1): " << hRingObservable_daughter_per_candidate_PtYCuts_integrated_err->GetBinContent(1) << std::endl;
    ///////////////////////////////////////////////////////////////

    // Setting the error bars:
        // Setting the errors for the 4 equivalent histograms:
    SetErrorsFromErrHist(hRingObservable_proxy_from_daughter_eq_defPtYCuts, hRingObservableRecoPtYCuts_Err);
    SetErrorsFromErrHist(hRingObservable_proxy_from_daughter_star_eq_defPtYCuts, hRingObservableRecoPtYCuts_Err);
    SetErrorsFromErrHist(hRingObservableRecoPtYCuts, hRingObservableRecoPtYCuts_Err);
    SetErrorsFromErrHist(hRingObservableStarRecoPtYCuts, hRingObservableRecoPtYCuts_Err);
    
        // Now for the non-pT cuts variations:
    SetErrorsFromErrHist(hRingObservableReco, hRingObservableReco_Err);
    SetErrorsFromErrHist(hRingObservable_proxy_from_daughter_eq_def, hRingObservableReco_Err);
    SetErrorsFromErrHist(hRingObservable_proxy_from_daughter_star_eq_def, hRingObservableReco_Err);

        // For the integrated plots:
    SetErrorsFromErrHist(hRingObservableRecoPtYCuts_integrated, hRingObservableRecoPtYCuts_integrated_Err);
    SetErrorsFromErrHist(hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated, hRingObservableRecoPtYCuts_integrated_Err);
    SetErrorsFromErrHist(hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated, hRingObservableRecoPtYCuts_integrated_Err);
    // Didn't include an hRingObservableStarRecoPtYCuts_integrated here, but whatever: it should be equivalent to the previous three here already!

    // The same for the ring observable using "per-candidate"-like error propagation:
    CopyTH1D(hRingObservable_proxy_from_daughter_star_eq_def, hRingObservableReco_with_err_per_candidate);
    CopyTH1D(hRingObservable_proxy_from_daughter_star_eq_defPtYCuts, hRingObservableRecoPtYCuts_with_err_per_candidate);
    CopyTH1D(hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated, hRingObservableRecoPtYCuts_integrated_with_err_per_candidate);

    SetErrorsFromErrHist(hRingObservableReco_with_err_per_candidate, hRingObservable_daughter_per_candidate_err);
    SetErrorsFromErrHist(hRingObservableRecoPtYCuts_with_err_per_candidate, hRingObservable_daughter_per_candidate_PtYCuts_err);
    SetErrorsFromErrHist(hRingObservableRecoPtYCuts_integrated_with_err_per_candidate, hRingObservable_daughter_per_candidate_PtYCuts_integrated_err);
    ...
    hDebugCounter_phi_star_sampler->Write();
        ...
        // The PtYCuts counters:
    hLambdaCounter_phi_Weighted_PtYCuts->Write();
    hLambdaCounter_phiRingAngles_Weighted_PtYCuts->Write();
    hProtonCounter_phiRing_Weighted_PtYCuts->Write();
    hProtonStarCounter_phiRing_Weighted_PtYCuts->Write();
    ...
    hRingObservable_TrueValuePtYCuts->Write();
    hRingObservable_NormPolTrueValuePtYCuts->Write();
    
    hRingObservableReco->Write();
    hRingObservableRecoPtYCuts->Write();
    hRingObservableNormPolRecoPtYCuts->Write();
    hRingObservableStarRecoPtYCuts->Write();
    hRingObservableStarNormPolRecoPtYCuts->Write();

    hRingObservable_proxy_from_daughter->Write();
    hRingObservable_proxy_from_daughter_eq_def->Write();
    hRingObservable_proxy_from_daughter_eq_defPtYCuts->Write();
    hRingObservable_proxy_from_daughter_eq_def2PtYCuts->Write();

    hRingObservable_proxy_from_daughter_star_eq_def->Write();
    hRingObservable_proxy_from_daughter_star_eq_defPtYCuts->Write();

        // Integrated observables:
    hRingObservable_TrueValuePtYCuts_integrated->Write();
    hRingObservable_NormPolTrueValuePtYCuts_integrated->Write();
    hRingObservableRecoPtYCuts_integrated->Write();
    hRingObservableNormPolRecoPtYCuts_integrated->Write();
    hRingObservable_proxy_from_daughter_eq_defPtYCuts_integrated->Write();
    hRingObservable_proxy_from_daughter_star_eq_defPtYCuts_integrated->Write();

    // The error-related histograms:
    hLambdaCounterSquared_pT_y_phi_Weighted->Write();
    ...
    hLambdaNeff_pT_y_phi_Weighted->Write();
    hLambdaNeff_minus_1_pT_y_phi_Weighted->Write();
    ...
    hRingObservableRecoPtYCuts_Err->Write();
    hRingObservableRecoPtYCuts_integrated_Err->Write();
    ...
    hRingObservableReco_with_err_per_candidate->Write();
    hRingObservableRecoPtYCuts_with_err_per_candidate->Write();
    hRingObservableRecoPtYCuts_integrated_with_err_per_candidate->Write();
        // Needed to get the error values:
    ...
    //     // 1D version for the counter-1:
    // hLambdaCounter_DeltaPhiJRingAngles_minus1->Write();
    ...
    std::cout << "Of those, " << duration_fetch.count() << " seconds were used to fetch data, " << pre_process_duration.count() << " seconds to pre-process for the parallel loop, and " << duration_resample.count() << " seconds to sample distributions." << std::endl;
    ...
    std::cout << "The average time for each sample was " << duration_resample.count() * 1./N_resamples << " seconds per sample, or " << duration_resample.count() * 1./N_threads << " seconds of total time per thread." << std::endl;
